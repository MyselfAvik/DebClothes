...
import { User, Mail, MapPin, Key, Trash, Plus, Shield, Navigation, AlertCircle, CheckCircle, RefreshCw } from 'lucide-react';
import toast from 'react-hot-toast';

const Profile = () => {
  const { user, addAddress, deleteAddress, requestChangePasswordOtp, verifyChangePasswordOtp } = useAuth();
  ...
  const handleAddAddressSubmit = async (e) => {
    ...
    try {
      ...
      const err2 = err.response?.data?.message || 'Failed to add address';
      setAddressError(err2);
      toast.error(err2);
    } finally {
            ...
            {showAddAddress && (
              <form onSubmit={handleAddAddressSubmit} className="mb-8 p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-[#0f172a]/20 space-y-4">
                <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400">New Address Form</h3>
                  <button
                    type="button"
            ...
            {user?.addresses && user.addresses.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {user.addresses.map((addr) => (
                  ...
                  >
                    <div className="space-y-1.5 text-slate-755 dark:text-slate-300">
