...
import { User, Mail, Lock, Sparkles, Eye, EyeOff, ArrowRight } from 'lucide-react-native';
...
export default function RegisterScreen() {
  ...
  const handleRealGoogleOAuth = async () => {
    ...
    try {
      ...
      showToast(err.response?.data?.message || err.message || 'Google sign-up failed', 'error');
    } finally {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      navigateTo('OTP_VERIFICATION', { email: email.trim(), purpose: 'Signup' });
    } catch (err) {
        ...
        >
          ...
          <View style={styles.headerContainer}>
            ...
            <Text style={[styles.brandTagline, { color: isDark ? '#a78bfa' : '#6b7280' }]}>
              JOIN OUR EXCLUSIVE RUNWAY CLUB
            </Text>
          </View>
          ...
          >
            <Text style={[styles.cardTitle, { color: isDark ? '#ffffff' : '#1e1b4b' }]}>
              Create New Account
            </Text>
            ...
            <View style={styles.fieldBlock}>
              ...
              >
                <User
                  size={17}
                  color={nameFocused ? (isDark ? '#c084fc' : '#7c3aed') : (isDark ? '#71717a' : '#9ca3af')}
